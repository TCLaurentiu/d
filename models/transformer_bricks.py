...
class LATRDecoderLayer(BaseTransformerLayer):
    def __init__(self,
        ...
        self.ffns = nn.ModuleList([MixerModel(
            d_model = 256,
            n_layer = 1,
            ssm_cfg=None,
...
class LATRTransformerDecoder(TransformerLayerSequence):
    def __init__(self,
        ...
        out_conv = torch.nn.Conv2d(in_channels=260, out_channels=2, kernel_size=1)

        out_avg = torch.nn.AdaptiveAvgPool2d(1)

        mamba_layers += [out_linear, unflatten, out_conv, out_avg]
        ...
        self.gflat_pred_layer = nn.Sequential(
            ...
            #input is batch_size * 260 * 90 * 120
            #output is batch_size * 2 * 1 * 1
