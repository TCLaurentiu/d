...
# input is batch_size x 258x90x120
# output is batch_size x 256x90x120
def _make_stack_3x3_convs(num_convs, in_channels, out_channels):
    """
    # shape after flatten is 258x10800
    flatten = torch.nn.Flatten(start_dim=2, end_dim=3)

    # shape after linear is 258x100
    linear = torch.nn.Linear(in_features=10800, out_features=100)

    mamba_layers = [flatten, linear]
    mamba_layers.append(
        MixerModel(
            d_model = 100,
            n_layer = num_convs,
    ...
    # shape after mamba layers is the same: 258x100
    out_linear = torch.nn.Linear(in_features=100, out_features=10800)
    unflatten = torch.nn.Unflatten(dim=2, unflattened_size=(90, 120))

    out_conv = torch.nn.Conv2d(in_channels=258, out_channels=256, kernel_size=1)

    mamba_layers += [out_linear, unflatten, out_conv]

    return nn.Sequential(*mamba_layers)
    """
    convs = []
